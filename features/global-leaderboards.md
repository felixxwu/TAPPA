...
| `scripts/cloud/firestore_board.gd` (`FirestoreBoard`) | Shared base (`extends Node`) for `Leaderboard` and `ChallengeLeaderboard`: `_documents_root`, `_board_entry_doc`, `_signed_in`, `_uid`, `_classify`, `_uid_from_path`, `_board_run_query` (top-N `runQuery`, parameterised on parent doc / collection id / order field / limit / offset / row-builder callable) and `_board_count` (a `runAggregationQuery` COUNT, parameterised on parent doc + the caller's own structured query). Each board still owns its own `_read_own`, `_row_from_document`, `_unavailable` and all POST_* write semantics — those differ in shape between the two boards. |
