...
| `counter_steer_speed` | 3.5 (`.tres`) | Rate limit (rad/s) when winding lock back OFF — target closer to center or across to the opposite lock from the current wheel angle. Real countersteering/slide-catching happens faster than winding new lock on, so this is tuned higher than `steer_speed` |
