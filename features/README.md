...
| [menu-showcase.md](menu-showcase.md) | **`MenuShowcase`/`MenuShowcaseCamera`** — the live 3D scenic background behind the hub's pages: a fixed-seed track + terrain the camera flies around and cuts between angles on. Phase-1 prototype only (one region) — see `todo/menu-background-showcase.md` for the full multi-region design |
