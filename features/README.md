...
| [engine-swap.md](engine-swap.md) | The Engine Swap as a MID-RUN power boost (BoostLibrary `"engine_swap"` → the `engine_power_mult` EFFECTS row on `peak_torque`), plus the legacy owned-car `swapped_engine` read path and the engine-detune power knob |
