...
1. **Locked** — `unlock.stat`'s lifetime counter hasn't crossed `unlock.threshold`
   yet. `SkillLibrary.is_unlocked(id, profile)` is false. Shown on the menu, named,
...
2. **Purchasable** — the threshold IS crossed, but the skill is not yet bought.
   `SkillLibrary.is_purchasable(id, profile)` is true (implies `is_unlocked`). The
   menu offers a Buy card, priced at `SkillLibrary.price_of(id)`, disabled when
   unaffordable — same idiom as the CAR page's Buy cards.
3. **Owned** — bought via `Save.buy_skill(id)`, recorded in
   `Save.profile[Save.KEY_BOUGHT_SKILLS]`. `Save.owns_skill(id)` is true.

**Equipping is a fourth, SEPARATE axis from owning** —
`Save.profile[Save.KEY_EQUIPPED_SKILLS]`, toggled by `Save.equip_skill` /
`Save.unequip_skill`, capped at `GameConfig.skill_max_equipped`
(`@export_group("Roguelike Skills")`; RR's own constant is `PERK_MAX_EQUIPPED = 3`).
An owned skill need not be equipped; the cap is enforced once, in `Save.equip_skill`,
never re-checked by a caller.

Every `buy_skill`/`equip_skill`/`unequip_skill` refusal leaves the profile
...
`RegionLibrary`. `unlock.stat` names a `LifetimeStats` id — the vocabulary a skill's
gate may use is exactly that table's keys
(`test_skill_library.gd -> test_every_unlock_stat_is_a_real_lifetime_stat` enforces
this against a typo).

A `Registry.Seam` (`SkillLibrary.override_for_test` / `reset`) lets a test swap in a
synthetic roster — per CLAUDE.md, a skill's price/threshold/existence is authored
data, so no test may pin the shipped table; `test_skill_library.gd` builds its own
fixture skills for every state-machine assertion and only reads the real `SKILLS` for
...
51 — a skill whose authored description promises an effect it doesn't have is a
...
the same shape as a boost's — and `SkillLibrary.effect_for(id)` reads those fields
live off `Config.data`, so a designer's retune lands on the next stage boot with no
code change. `SkillLibrary.equipped_effects(profile)` returns them in
...
modifier path"), so a new skill adds an `EFFECTS` row and a `GameConfig` field —
never a bespoke read at some call site.

| Skill | `EFFECTS` key → config field | Who reads the field |
| --- | --- | --- |
...
### Why every skill row carries `reseed`

Skills differ from boosts in WHAT they target. Every pre-existing `EFFECTS` row moves
...
stage boot. **Skill rows target global tunables that have no such re-seed.** Nothing
calls `Config.reset()` between stages, so without help a coin radius multiplied on
stage 1 would be multiplied again on stage 2, and un-equipping the skill would never
...
only by the funnel — so `DamageModel.regen` knows nothing about skills; it reads a
...
`DAMAGE_TAKEN` gates a skill unlock, so letting it run backwards could un-unlock a
skill the player had already earned.

## The menu

`HubShell.View.SKILLS`, reached from `View.MAIN`'s "Skills" card. One `CardCarousel` card
per `SkillLibrary.all()` entry ([card-carousel.md](card-carousel.md)) — a locked card
(disabled, names its gate via `SkillLibrary.unlock_label` — imperatively phrased,
"take 300 damage", through `LifetimeStats.goal_for`), a Buy card, or an
