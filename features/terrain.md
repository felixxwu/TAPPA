...
- `layers: Array[TerrainLayer]` — each layer is a (`wavelength_m`,
  ...
  (`WorldRuntime.layers_match` guard — was shared with `overworld.gd`, deleted in the
  roguelike pivot; `world.gd` is its only caller now), so the hill shape follows the live config.
  **Per-event override:** an event may set any of the 6 flat keys
  ...
  event's override — see `StageConfig.apply_event_config`, [region-runs.md](region-runs.md)).
...
- `load_radius: int` — the **live** ring radius in chunks; `(2*load_radius+1)²` chunks
  ...
  a bounded, streamed world could raise it. Cost is quadratic — see
  [terrain-streaming.md](terrain-streaming.md).
...
- **Params** are pushed from `GameConfig` (`apply_cliffs`) onto the manager by
  ...
  (runtime per-event scale, written by `StageConfig.apply_event_config` from
...
`TerrainManager` also carries a **bounded, streamed** world mode — built for the drivable
overworld hub that the roguelike pivot deleted. The consumer (the hub) is gone, but
`TerrainManager`'s side of the seam is real, live, headless-tested infrastructure that
survives it — `load_radius`, `stream_on_miss`, `chunk_source`, `road_source`, `pad_source`,
`region_source`, `set_bounds`/`apply_overworld_bounds`, cache eviction and budgeted spawning.
It is a set of **additive, off-by-default opt-ins**: leave it alone and behaviour is
bit-identical to a stage, and nothing in `main.tscn` touches it. See
**[terrain-streaming.md](terrain-streaming.md)** for the full seam, why it is kept rather
than deleted, and the coastline edge taper referenced below.
...
**Bounded-world mode** (dormant, see [terrain-streaming.md](terrain-streaming.md)): the
boundary is the coastline taper — see "The coastline edge taper" there. Still no
...
## Stage-based hilliness and curviness — retired

A region run drives 8 stages back to back, and later stages read as harder
terrain: hillier and curvier than the first. This USED to be a runtime
multiplier (`StageConfig.stage_scale`, keyed on `RunSession.events_completed()`)
layered on top of a rally's authored event — **deleted 2026-09**
(`todo/region-stage-slots-redesign.md`): each of a region's 8 stage SLOTS now
authors its own fixed `terrain_layer*_amplitude`/`straightness` directly (no
runtime scaling on top), so a slot's terrain is exactly what's written in
`RegionStageLibrary.STAGES` for it. See `features/region-stage-library.md`.

The `GameConfig.stage_hilliness_scale_{min,max}` /
`stage_curviness_scale_{min,max}` fields and `StageConfig.stage_scale` itself
are gone with it — there is no post-pivot reader left.

## Related config

`terrain_layer{1,2,3}_{wavelength,amplitude}`, `terrain_tile_per_meter`. For the overworld:
