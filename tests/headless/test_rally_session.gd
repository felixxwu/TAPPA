...
#
# THE SUBJECT OF THIS BLOCK IS **StageConfig**, NOT RallySession — the writer was
# extracted to scripts/stage_config.gd and only these tests' address stayed here.
# If you are deleting this file because RallySession is gone, MOVE this block (and
# the waterline block below it) into a test_stage_config.gd first: StageConfig is
# the config seam every kind of run goes through and it survives.
...
func test_an_active_events_resolved_config_equals_its_canonical_config() -> void:
	...
	var canonical := StageConfig.canonical_event_config(event)
