...
func _make_carpark_modal(build_body: Callable, build_footer := Callable()) -> Control:
	...
	# MenuPage.open_modal owns the hosting — its own CanvasLayer at MODAL_LAYER, below
	# ConfirmPopup, claiming the screen. Read its header for the two bugs that hand-rolling
	# this cost (an invisible modal in world-menu mode, and a 3D panel eating its clicks).
	var page := MenuPage.open_modal(_hq, {"dim": true, "margin": 16.0, "padding": 20})
