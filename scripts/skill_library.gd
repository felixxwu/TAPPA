...
# Tests: tests/headless/test_skill_library.gd — extend in the same change.
#
# THE PERK CATALOGUE (todo/roguelike-pivot.md "Skills — a straight lift from RR", stage
...
# a specific catalogue entry existing" — skill *definitions* are authored data, exactly
...
#                  is_unlocked() false.
#   PURCHASABLE  — the threshold IS crossed, but the skill is not yet bought.
#                  is_purchasable() true (implies is_unlocked()).
#   OWNED        — bought (Save.KEY_BOUGHT_SKILLS, via Save.buy_skill). Save.owns_skill().
# Equipping is a SEPARATE step from owning (Save.KEY_EQUIPPED_SKILLS, via
# Save.equip_skill/unequip_skill), capped at GameConfig.skill_max_equipped (RR's
# PERK_MAX_EQUIPPED = 3) — an owned skill need not be equipped, and the cap is
# enforced once, in Save.equip_skill, not re-checked by every caller.
#
...
# parallel modifier path"), so a skill that wants a new kind of effect adds an EFFECTS
# row + a GameConfig field, never a bespoke read at some call site.
#
# WHERE EACH SKILL ACTUALLY LANDS — the field is the contract, the number is tunable:
...
# contract `test_skill_library.gd -> test_every_unlock_stat_is_a_real_lifetime_stat`
...
# override_for_test() rather than reading SKILLS below.
const SKILLS: Array[Dictionary] = [
	...
	},
	{
		...
		"effect_fields": {"damage_regen_set": "skill_heal_hp_per_s"},
	},
	{
		...
		"effect_fields": {"fast_bonus_money_mult": "skill_fast_bonus_mult"},
	},
	{
		...
		"effect_fields": {"coin_count_mult": "skill_coin_count_mult"},
	},
	{
		...
		"effect_fields": {"impact_damage_mult": "skill_damage_mult"},
	},
	{
		...
		"effect_fields": {"target_pace_add": "skill_target_pace_add"},
	},
	{
		...
		"effect_fields": {"stage_money_base_add": "skill_stage_money_add"},
	},
]

static var _seam := Registry.Seam.new(SKILLS)

static func all() -> Array[Dictionary]:
	return _seam.all()

static func override_for_test(skills: Array[Dictionary]) -> void:
	_seam.override_for_test(skills)
...
# The stat id and threshold a skill's unlock gate names. {} for an unknown id.
...
# Unlocked, but not yet bought — the state that puts a skill in front of the player
...
static func is_purchasable(id: String, profile: Dictionary) -> bool:
	if not is_unlocked(id, profile):
		return false
	return not (profile.get(Save.KEY_BOUGHT_SKILLS, []) as Array).has(id)


# Display text for a locked card's gate — IMPERATIVE phrasing ("take 300 damage"),
# via LifetimeStats.goal_for: an instruction the player can act on, not the stats
# page's read-out label ("Damage taken: 300"). Falls back to the raw stat id if it
# names something LifetimeStats does not declare (a content bug this reads as a
# visible ugly label rather than a crash, which is what
# test_every_unlock_stat_is_a_real_lifetime_stat exists to catch before it ships).
static func unlock_label(id: String) -> String:
	var unlock := unlock_of(id)
	var stat := String(unlock.get("stat", ""))
	return LifetimeStats.goal_for(stat, int(unlock.get("threshold", 0)))
...
# Every EQUIPPED skill's effect, in the exact shape UpgradeLibrary.active_effects reads
# ({"id": String, "effect": Dictionary}) so world.gd can append it straight onto the
# fielded car's `boosts` list.
#
# OWNED **AND** EQUIPPED. The cap (GameConfig.skill_max_equipped) is enforced once, in
# Save.equip_skill, but the ownership cross-check is repeated here on purpose: an
# equipped list that outlived its purchase (a hand-edited save, a schema migration, a
# refund path added later) must not quietly hand out a free effect. A skill with no
...
static func equipped_effects(profile: Dictionary) -> Array:
	...
	var out: Array = []
	for id in (profile.get(Save.KEY_EQUIPPED_SKILLS, []) as Array):
		var skill_id := String(id)
		if not bought.has(skill_id):
			continue
		var effect := effect_for(skill_id)
		if effect.is_empty():
			continue
		out.append({"id": skill_id, "effect": effect})
	return out
