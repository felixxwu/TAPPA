...
# Tests: tests/headless/test_lake_field.gd, tests/headless/test_track_cache.gd, tests/headless/test_track_generator.gd — extend in the same change. These are the PRIMARY ones, not all of them: before you change behaviour here, `grep -rn 'TrackGenerator' tests/headless/` and read the assertions that pin what you are about to change (7 test files touch this script).
