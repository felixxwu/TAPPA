...
#
# `drivetrain_ids` are pseudo-ids of the shape "drivetrain:<DriveMode int>"
# (RunSession derives them from drivetrain_choices()) that a mode opting into the
...
func boost_choices(_stage_index: int, _count: int = -1, _drivetrain_ids: Array = []) -> Array:
	return []
