...
- **The overworld map** — `hq_map_table.gd`, `hq_table.gd`, `rally_detail.gd`, the
  ...
  `hq.gd` (`_refresh_map_pins`, `_make_pin`). `hq.gd` is 3563 lines and is the
  single largest surface this pivot touches.
...
`damage.md`, `rally-roster.md`, `menus.md`, `hq.md`, and `features/README.md`'s
index.
...
4. **Region select + linear unlock**, replacing the map table. Nav test included.
5. **In-run boosts + repair pick** between stages, wiped on run end.
6. **Meta shop**: boost levels, then car purchasing.
7. **Lifetime stats, then perks** (perks depend on stats).
8. **Delete the old career** — rivals, map, parts, prize rallies — and the save
   migration. Last, so the game is never non-functional mid-pivot.
9. **Docs and full suite.** `features/` rewritten, one full `./run_tests.sh`.

Dependencies worth stating explicitly, per `CLAUDE.md`'s todo rules: 5 depends on
3; 7's perks depend on 7's stats; 8 depends on everything; the save migration in
8 cannot be written until 4–7 have settled what the profile holds.
