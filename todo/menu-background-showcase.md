...
Also disabled terrain **collision** entirely (`floor_tm.collision_ring = -1000`):
nothing in this scene ever touches the ground (no car, no player), so building a
`HeightMapShape3D` per chunk was pure waste. Smaller win than hoped (~200ms) —
most of the remaining corridor+cache cost turned out to be per-vertex height/
colour/UV2 computation and LOD mesh building, not collision.

## Follow-up 2: the CI-baked cache — built, then corrected against real numbers

The user then asked for the full CI-baked-array-cache idea to actually be
implemented, plus "reuse anything from the track cache". Delivered as
`MenuShowcaseCache` (`scripts/menu_showcase_cache.gd`), committed as
`data/menu_showcase_cache.res` (3.4 MB), generated by
`tools/generate_menu_showcase_cache.gd` via `cache_menu_showcase.sh` — the same
shape as `TrackCache`/`cache_tracks.sh`, and directly REUSING
`TrackGenerator.rebuild_from_pieces` (the exact no-search reconstruction
`TrackCache.lookup()` uses) and `TrackCache.terrain_fingerprint(cfg)` (folded
into the cache's own version tag) rather than re-deriving either. See
[features/menu-showcase.md](../features/menu-showcase.md) → "`MenuShowcaseCache`"
and "Build performance" for the full mechanism and the final numbers.

**The first version was wrong, and the numbers caught it before it shipped.** A
first pass cached each segment's WHOLE `TerrainManager._chunk_cache` verbatim
(heights + prebaked LOD meshes) — the full chunk-data store this file's earlier
section flagged as "harder, not-yet-designed." Generating it produced a **59 MB**
file, and measuring the cache-HIT build showed it was **slower**, not faster
(3.6s vs 5.8s live) — deserializing hundreds of prebaked `Mesh` resources back off
disk cost nearly as much as building them live, confirming the ORIGINAL
per-phase measurement (chunk spawning was 1% of total build time, never the
bottleneck) the hard way. **Corrected**: `MenuShowcaseCache` now caches only the
cheap-to-store, expensive-to-*compute* parts — the track shape (`pieces`), the
shared bake dictionaries, and the foliage scatter points — and deliberately never
touches per-chunk data. `corridor + chunk cache` stays a permanently live phase.

Final measured result: **18,376 ms (original) → 5,725 ms (bake-sharing + no
collision) → 3,546 ms (+ `MenuShowcaseCache`)** — roughly **5.2x** overall, for a
3.4 MB committed asset. `corridor + chunk cache` is now ~83% of the remaining
total; the one lever left untried is shrinking `_CORRIDOR_LEASH_M` (currently
50m, sized for a real stage's off-track leash — this scene's fixed-offset camera
needs far less), left alone because it trades runtime cost against a real risk of
a visible hole/pop-in at a shot's frame edge that nobody authoring this blind
should decide alone.
