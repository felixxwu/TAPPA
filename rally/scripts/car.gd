...
# Frozen-shading bake state (see bake_shading / restore_shading). While a parked
# car is frozen its world-space normals don't change, so its fake per-vertex
# lighting is baked into the mesh vertex colours once and the live shader calc is
# switched off — saving the per-vertex lighting work every frame for every parked
# car. _shading_restore remembers each touched mesh's original mesh + material so
# the bake is fully reversible.
var _shading_baked := false
var _shading_restore: Array = []  # [{mi, mesh, mat}] captured at bake time
...
# Freeze the fake per-vertex lighting into the car's meshes. Called when the car is
# parked as a static prop (HQ car-park lineup / lift, podium standings) — a frozen
# car never moves, so its world-space normals (and therefore its shading) are
# constant and don't need recomputing every frame. This bakes the current shading
# into each lit mesh's vertex COLOUR (which the shader already multiplies in) and
# sets light_amount to 0 on a per-instance copy of the material, so the shader's
# vertex() stage skips the lighting maths for this car (see ps1_models_lit.gdshader).
# Mirrors what the terrain does at generation time (terrain_manager._bake_light).
#
# Bakes at the car's CURRENT world pose, so call it AFTER the car has settled and
# been positioned. Only valid while the car holds that orientation: a pure
# translation (e.g. the HQ lift rising) leaves normals unchanged, but anything that
# ROTATES the car in world space (e.g. the podium turntable) must stay live, so it
# must not be baked. Idempotent; reverse with restore_shading.
func bake_shading() -> void:
	if _shading_baked:
		return
	var shader: Shader = load("res://shaders/ps1_models_lit.gdshader")
	for node in find_children("*", "MeshInstance3D", true, false):
		...
		var mat := mi.get_surface_override_material(0) as ShaderMaterial
		if mat == null or mat.shader != shader:
			continue  # not a fake-lit car mesh (e.g. an MX-5 sub-mesh left imported)
		var baked := _bake_lit_mesh(mi.mesh, mi.global_basis)
		if baked == null:
			continue
		# Per-instance material copy with the live lighting disabled, so switching it
		# off here never affects the player's car (car.tscn materials are shared
		# sub-resources across instances).
		var flat := mat.duplicate() as ShaderMaterial
		flat.set_shader_parameter("light_amount", 0.0)
		_shading_restore.append({"mi": mi, "mesh": mi.mesh, "mat": mat})
		mi.mesh = baked
		mi.set_surface_override_material(0, flat)
	_shading_baked = true


# Undo bake_shading: restore each touched mesh's original mesh + shared material so
# the live per-vertex lighting resumes. No-op if nothing was baked.
func restore_shading() -> void:
	if not _shading_baked:
		return
	for entry in _shading_restore:
		var mi := entry["mi"] as MeshInstance3D
		if is_instance_valid(mi):
			mi.mesh = entry["mesh"]
			mi.set_surface_override_material(0, entry["mat"])
	_shading_restore.clear()
	_shading_baked = false


# Build a copy of `src` whose vertex COLOUR carries the fake per-vertex lighting for
# the given world-space basis, mirroring shaders/ps1_models_lit.gdshader's vertex()
# (hemisphere ambient + one directional sun) on the CPU. The shader fragment already
# multiplies COLOR.rgb in, so a flat (light_amount 0) material then renders the baked
# look at no per-frame cost. Returns null if the source has no usable normals.
func _bake_lit_mesh(src: Mesh, world_basis: Basis) -> ArrayMesh:
	var cfg: GameConfig = Config.data
	var light_dir := cfg.sun_direction.normalized()
	var out := ArrayMesh.new()
	var baked_any := false
	for s in src.get_surface_count():
		var arrays := src.surface_get_arrays(s)
		# Untyped: surface_get_arrays leaves unused channels null, and assigning null
		# to a typed Packed*Array variable is a runtime error (box meshes carry no
		# vertex colour, so ARRAY_COLOR is null until we add it below).
		var verts = arrays[Mesh.ARRAY_VERTEX]
		var normals = arrays[Mesh.ARRAY_NORMAL]
		if normals != null and verts != null and normals.size() == verts.size():
			var existing = arrays[Mesh.ARRAY_COLOR]
			var has_existing: bool = existing != null and existing.size() == verts.size()
			var colors := PackedColorArray()
			colors.resize(verts.size())
			for i in verts.size():
				var wn: Vector3 = (world_basis * normals[i]).normalized()
				var hemi := wn.y * 0.5 + 0.5
				var ambient := cfg.ground_color.lerp(cfg.sky_color, hemi)
				var ndl := maxf(wn.dot(light_dir), 0.0)
				var lit := Color(
					ambient.r + cfg.sun_color.r * ndl,
					ambient.g + cfg.sun_color.g * ndl,
					ambient.b + cfg.sun_color.b * ndl)
				var v_light := Color(1, 1, 1).lerp(lit, cfg.car_light_amount)
				var base: Color = existing[i] if has_existing else Color(1, 1, 1)
				colors[i] = Color(base.r * v_light.r, base.g * v_light.g, base.b * v_light.b, base.a)
			arrays[Mesh.ARRAY_COLOR] = colors
			baked_any = true
		out.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES, arrays)
		# Keep any material the source mesh carried on its own surfaces (the lit
		# car material is an instance override, handled by the caller).
		var sm := src.surface_get_material(s)
		if sm != null:
			out.surface_set_material(s, sm)
	return out if baked_any else null
